...
- **Authorization-scope vs. acted-on IDs (IDOR by ID-array smuggling)**: when an endpoint authorizes a request-named scope (`form_id`, `list_id`, `parent_id`) but then mutates/reads a *separate* attacker-supplied ID array (`entries[]`, `ids[]`, `submission_ids[]`), the cap check on the scope does NOT protect the IDs. A user scoped to Form A passes the policy, then smuggles Form B's IDs and the unscoped `whereIn('id', $ids)->delete()` hits them. Fix: re-scope the IDs (`->where('scope_id', $authorized)->whereIn('id', $ids)`) before acting, OR authorize against the *fetched rows'* real scope. **Branch-asymmetry is the tell**: in a bulk `action_type` dispatcher, compare every branch — the status/favorite branch usually scopes correctly while the delete branch passes raw IDs. Cascading meta/detail/log/order deletes need the same scope filter. (fluent-forms WPScan req 11316830, sibling of CVE-2026-5396 — the `other.delete_permanently` branch of `SubmissionService::handleBulkActions` deleted by raw `entries[]`.)
...
- Run a bulk-action branch-symmetry pass: enumerate every handler that dispatches on an `action_type`/`bulk_action` and reads an ID array from the request. For EACH branch, confirm it applies the same scope filter (`where('form_id'/'list_id'/...)`) before the IDs are used; flag any branch (typically delete) that omits it. Trace cascading deletes for the same gap. Whenever a known authz CVE is in scope, explicitly sweep its sibling code paths for the same class — do not assume the original patch covered siblings.

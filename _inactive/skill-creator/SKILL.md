...
name: skill-creator
description: >
  Design and create new reusable Claude Code skills. Use when you want to turn a repeatable
  workflow, review process, or checklist into an invocable skill that can be called with
  /skill-name. Also use when asked "create a skill", "make this reusable", "save this as a skill",
  or "turn this into a tool". Writes SKILL.md with correct frontmatter, splits criteria into
  references/ files, and registers the skill in the global agent-skills repo README.
when_to_use: >
  When the user wants to save a workflow as a reusable skill, after a review session
  the user wants to repeat, or when building a new automated check or generator.
context: fork
...
| `bug-fix` | Bug-handling: fix one known bug, or sweep (`find` mode) for new bugs with finder/verifier feedback loop |
...
| `pre-merge-review` | Single canonical pre-merge review — parallel detectors + sequential pattern passes, with `light` mode for tiny diffs. Re-callable. |
| `new-feature` | End-to-end composite feature workflow with approval gates |
| `rigorous-coding-workflow` | Principles layer (plan/verify/lessons) underneath the operational skills above |
